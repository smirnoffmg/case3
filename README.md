...
uv run case3 run "Список сотрудников с email, лимит 10" --stub
```

С реальной моделью (см. [настройки](#настройки)):

```bash
uv run case3 run "Показать 5 организаций"
```
...
Флаг `--stub` у `case3 run` принудительно включает заглушку LLM (без сети).
...
В UI снимите галочку "Stub LLM", если в `.env` настроена реальная модель.
...
| ----------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENAI_API_KEY`  | не задан      | Ключ API. Если пусто, используется **заглушка** (`StubLLMClient`), без вызова модели. Для локального сервера можно указать любое непустое значение, например `local`. |
...
| --------------------------------- | -------------------- | --------------- | ----------- |
| Нет `OPENAI_API_KEY` или `--stub` | Заглушка             | Да              | Нет         |
| Ключ задан, `USE_LLM=true`        | LangChain, вызов API | Да              | Да          |
| Ключ задан, `USE_LLM=false`       | LangChain, вызов API | Да              | Нет         |
